...
class CodableTests: XCTestCase {
    ...
    func testArrayCodable() {
        ...
        let json = try! JSONDecoder().decode(GSON.self, from: data)
        ...
        XCTAssertEqual(json.arrayValue[0].bool, true)
        XCTAssertEqual(json.array!.last!.bool, true)
        let jsonList = try! JSONDecoder().decode([GSON].self, from: data)
    ...
    func testDictionaryCodable() {
        ...
        XCTAssertEqual(json["number"].doubleValue, 9823.212)
        XCTAssertEqual(json["list"].arrayObject is [NSNumber], true)
        XCTAssertEqual(json["object"]["sub_number"].doubleValue, 877.2323)
        XCTAssertEqual(json["bool"].bool, true)
        let jsonDict = try! JSONDecoder().decode([String: GSON].self, from: data)
